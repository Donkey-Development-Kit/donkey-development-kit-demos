...
Run:
    export DONKEY_LLM_PROXY_URL / _CLIENT_ID / _CLIENT_SECRET   # see demo 01
    pip install "donkey-kit[langgraph]"   # + any frameworks you want
...
# (donkey attribute, method name, takes-model-arg, expected-native-type hint)
...
def main() -> None:
    if not _configured():
        print(__doc__)
        print(">> Set the three DONKEY_LLM_PROXY_* env vars to construct adapters.")
        return
    ...
    for attr, method, takes_model, hint in ADAPTER_CALLS:
        label = f"{attr:<{width}}"
        try:
            adapter = getattr(donkey, attr)          # ImportError if extra missing
            ...
            print(f"{label}  not installed  (pip install 'donkey-kit[{attr}]')")
